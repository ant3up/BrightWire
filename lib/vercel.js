...
class VercelHelper {
  ...
  async getDeploymentByBranch(branchName) {
    try {
      ...
      const url = this.teamId 
        ? `https://api.vercel.com/v6/deployments?projectId=${this.projectId}&teamId=${this.teamId}&limit=20`
        : `https://api.vercel.com/v6/deployments?projectId=${this.projectId}&limit=20`;
      ...
      // Find deployment with matching git branch
      const deployment = deployments.find(dep => 
        dep.git?.ref === branchName || 
        dep.meta?.gitBranch === branchName ||
        dep.name === branchName
      );
   ...
   */
  async waitForDeploymentByBranch(branchName, timeoutMs = 180000, pollIntervalMs = 4000) {
